...
use crate::coordination_materialized_store::{
    CoordinationCompactionWriteRequest, CoordinationReadModelsWriteRequest,
    CoordinationStartupCheckpointWriteRequest, StoreBackedCoordinationMaterializedStore,
};
    ...
    T: CoordinationJournal + CoordinationCheckpointStore + ?Sized,
{
    ...
    StoreBackedCoordinationMaterializedStore::new(root, store).write_read_models_mut(
    ...
    if should_compact {
        StoreBackedCoordinationMaterializedStore::new(root, store).write_compaction_mut(
            CoordinationCompactionWriteRequest {
    ...
    if let Some(canonical_snapshot_v2) = materialization.canonical_snapshot_v2.as_ref() {
        ...
        StoreBackedCoordinationMaterializedStore::new(root, store).write_startup_checkpoint_mut(
