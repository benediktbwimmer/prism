...
use prism_coordination::{
    execution_overlays_from_tasks, plan_graph_from_coordination, snapshot_plan_graphs, Artifact,
    ArtifactReview, CoordinationSnapshot, CoordinationTask, Plan, RuntimeDescriptor,
    RuntimeDescriptorCapability, WorkClaim,
};
...
fn authoritative_shared_coordination_ref_heads(
    root: &Path,
) -> Result<BTreeMap<String, String>> {
    let mut refs = BTreeMap::new();
...
fn authoritative_shared_coordination_state_key(root: &Path) -> Result<Option<String>> {
    ...
    Ok(Some(blake3::hash(canonical.as_bytes()).to_hex().to_string()))
}
...
pub(crate) fn initialize_shared_coordination_ref_live_sync(root: &Path) -> Result<()> {
    ...
    record_observed_shared_coordination_head(root, authoritative_shared_coordination_state_key(root)?);
    Ok(())
}
...
fn sync_task_shard_refs(
    ...
    let desired = tasks
        .iter()
        .cloned()
        .fold(BTreeMap::<String, Vec<CoordinationTask>>::new(), |mut shards, task| {
            shards
...
fn sync_claim_shard_refs(
    ...
    let desired = claims
        .iter()
        .cloned()
        .fold(BTreeMap::<String, Vec<WorkClaim>>::new(), |mut shards, claim| {
            shards
...
fn load_records_from_ref_lenient<T, F>(
    root: &Path,
    ref_name: &str,
    parse: F,
) -> Result<Vec<T>>
    ...
    F: Fn(&SharedCoordinationRefContents) -> Result<Vec<(String, T)>>,
{
    ...
    Ok(parse(&contents)?.into_iter().map(|(_, value)| value).collect())
}
...
fn load_authoritative_shared_coordination_ref_state(
    ...
    if summary_state.is_none() && task_shards.is_empty() && claim_shards.is_empty() {
        return Ok(None);
    }
    ...
    state.snapshot.tasks.sort_by(|left, right| left.id.0.cmp(&right.id.0));
    state.snapshot.claims.sort_by(|left, right| left.id.0.cmp(&right.id.0));
...
fn desired_runtime_descriptors(
    ...
    if !refresh_local_runtime {
        return Ok(existing.unwrap_or(&[]).to_vec());
    }
...
mod tests {
    ...
    fn shared_coordination_ref_retries_stale_head_and_merges_disjoint_changes() {
        ...
        let artifact_ids = loaded
            ...
            .collect::<BTreeSet<_>>();
        assert!(artifact_ids.contains("artifact:a"), "artifact ids: {artifact_ids:?}");
        assert!(artifact_ids.contains("artifact:b"), "artifact ids: {artifact_ids:?}");
    ...
    fn shared_coordination_publish_patch_only_stages_changed_task_payload() {
        ...
        assert!(
            patch.upserts.is_empty(),
            "task-only edits should not require a summary-ref upsert once task authority lives on shard refs; saw {:?}",
            patch.upserts
        );
        ...
        let task_shard_head_after =
            super::run_git(&root, &["rev-parse", &task_shard_ref]).unwrap();
        assert_eq!(
            head_before.trim(),
            head_after.trim(),
            "task-only edits should leave the summary ref head unchanged"
        );
        assert_ne!(
    ...
    fn shared_coordination_ref_diagnostics_report_manifest_and_history() {
        ...
        let task_relative_path = super::run_git(&root, &["ls-tree", "-r", "--name-only", &task_shard_ref])
            .unwrap()
            .lines()
            .find(|path| path.ends_with(&super::snapshot_file_name("coord-task:shared-diagnostics")))
            .map(str::to_string)
        ...
        .unwrap();
        let runtime_relative_path = super::run_git(&root, &["ls-tree", "-r", "--name-only", &ref_name])
            .unwrap()
            .lines()
            .find(|path| {
                path.ends_with(&super::snapshot_file_name(
                    &workspace_identity_for_root(&root).worktree_id,
                ))
            })
            .map(str::to_string)
            .expect("summary ref should contain the runtime descriptor");
        let raw_runtime_descriptor: serde_json::Value = serde_json::from_str(
            &super::run_git(
                &root,
                &["show", &format!("{ref_name}:{runtime_relative_path}")],
            )
            .unwrap(),
        )
        .unwrap();
