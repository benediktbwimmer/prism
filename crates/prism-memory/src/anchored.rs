...
use crate::types::{
    EpisodicMemorySnapshot, MemoryEntry, MemoryId, MemoryKind, RecallQuery, ScoredMemory,
};
...
impl AnchoredMemory {
    pub(crate) fn new(
        ...
        Self {
            ...
            id_prefix,
            supported_kinds,
            state: RwLock::new(AnchoredState::default()),
        }
    }
    ...
    pub(crate) fn entry(&self, id: &MemoryId) -> Option<MemoryEntry> {
        self.state
            .read()
            .expect("anchored memory lock poisoned")
            .entries
            .get(id)
            .cloned()
    }
    ...
    pub(crate) fn snapshot(&self) -> EpisodicMemorySnapshot {
        let state = self.state.read().expect("anchored memory lock poisoned");
    ...
    pub(crate) fn from_snapshot(
        ...
        let memory = Self::new(module_name, id_prefix, supported_kinds);
        let mut state = memory.state.write().expect("anchored memory lock poisoned");
    ...
    pub(crate) fn store(&self, mut entry: MemoryEntry) -> Result<MemoryId> {
        ...
        let mut state = self.state.write().expect("anchored memory lock poisoned");
    ...
    pub(crate) fn recall(&self, query: &RecallQuery) -> Result<Vec<ScoredMemory>> {
        ...
        let mut results = candidate_ids
            ...
            .filter_map(|id| {
                ...
                score_memory(self.module_name, &id, entry, query)
            })
            .collect::<Vec<_>>();
...
fn restore_entry(state: &mut AnchoredState, mut entry: MemoryEntry) {
    entry.anchors = dedupe_anchors(entry.anchors);
    entry.trust = clamp_unit(entry.trust);
...
fn recall_candidates(state: &AnchoredState, query: &RecallQuery) -> HashSet<MemoryId> {
...
fn replace_anchor(state: &mut AnchoredState, old_anchor: &AnchorRef, replacements: &[AnchorRef]) {
...
fn remove_memory(state: &mut AnchoredState, memory_id: &MemoryId) {
