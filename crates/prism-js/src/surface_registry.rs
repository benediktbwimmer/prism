...
function __prismCompilerHostWrite(methodPath, payload) {{
  return __prismHost("__compilerWrite", {{ methodPath, payload }});
}}
...
function __prismCompilerInvoke(methodPath, receiver, args = []) {{
  ...
  switch (methodPath) {{
    case "prism.work.declare": {{
      ...
      return __prismWrapCompilerResult(methodPath, __prismCompilerHostWrite(methodPath, {{ input }}));
    }}
    case "prism.claim.acquire": {{
      const input = __prismCompilerValidateInput(methodPath, spec, args);
      const coordinationTaskId = input?.coordinationTaskId ?? input?.coordination_task_id;
      return __prismWrapCompilerResult(methodPath, __prismCompilerHostWrite(methodPath, {{
        input: {{
    ...
    case "prism.claim.release": {{
      const claim = receiver ?? args[0];
      return __prismWrapCompilerResult(methodPath, __prismCompilerHostWrite(methodPath, {{ claim }}));
    }}
    ...
    case "prism.artifact.supersede": {{
      ...
      return __prismWrapCompilerResult(methodPath, __prismCompilerHostWrite(methodPath, {{ artifact }}));
    }}
    ...
    case "prism.coordination.openPlan": {{
      ...
      return __prismWrapCompilerResult(methodPath, __prismCompilerHostWrite(methodPath, {{ planId: planId.trim() }}));
    }}
    case "prism.coordination.openTask": {{
      ...
      return __prismWrapCompilerResult(methodPath, __prismCompilerHostWrite(methodPath, {{ taskId: taskId.trim() }}));
    }}
    case "plan.update": {{
      const input = __prismCompilerValidateInput(methodPath, spec, args);
      return __prismWrapCompilerResult(methodPath, __prismCompilerHostWrite(methodPath, {{
    ...
    case "plan.archive": {{
      return __prismWrapCompilerResult(methodPath, __prismCompilerHostWrite(methodPath, {{ plan: receiver }}));
    }}
    case "plan.addTask": {{
      const input = __prismCompilerValidateInput(methodPath, spec, args);
      return __prismWrapCompilerResult(methodPath, __prismCompilerHostWrite(methodPath, {{
    ...
    case "task.dependsOn": {{
      return __prismCompilerHostWrite(methodPath, {{
        task: receiver,
        dependsOn: args[0],
        kind: args[1]?.kind,
      }});
