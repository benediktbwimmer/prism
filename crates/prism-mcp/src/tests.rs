...
fn git_execution_policy_start_rejects_main_branch_and_records_failure() {
    ...
    let task = host
        ...
        .unwrap();
    assert_eq!(task.task.lifecycle_status, prism_ir::TaskLifecycleStatus::Pending);
    assert_eq!(task.status, prism_ir::EffectiveTaskStatus::Pending);
    assert_eq!(
...
fn git_execution_policy_start_combines_requested_mutation_and_git_execution_record() {
    ...
    let task = server
        ...
        .unwrap();
    assert_eq!(task.task.lifecycle_status, prism_ir::TaskLifecycleStatus::Active);
    assert_eq!(task.status, prism_ir::EffectiveTaskStatus::Active);
    assert_eq!(
        task.task.git_execution.status,
...
fn git_execution_policy_defaults_to_off_modes() {
    ...
    let plan = host
        .store_coordination(
            ...
            PrismCoordinationArgs {
                ...
                payload: json!({
                    "title": "Git execution",
                    "goal": "Keep git execution opt-in by default"
                }),
                task_id: None,
            },
        )
    ...
    let plan = host
        .current_prism()
        .plan(&prism_ir::PlanId::new(
            plan.state["id"].as_str().unwrap().to_string(),
        ))
        .unwrap();
    assert_eq!(
        plan.plan.policy.git_execution.start_mode,
        prism_coordination::GitExecutionStartMode::Off
    );
    assert_eq!(
        plan.plan.policy.git_execution.completion_mode,
        prism_coordination::GitExecutionCompletionMode::Off
    );
    assert!(!plan.plan.policy.git_execution.require_task_branch);
    assert_eq!(plan.plan.policy.git_execution.target_branch, "main");
}

#[test]
fn git_execution_policy_task_create_rejects_direct_in_progress_creation() {
    let root = init_git_workspace("task/git-execution-task-create");
    let session = index_workspace_session_with_options(
        &root,
        WorkspaceSessionOptions {
            coordination: true,
    ...
    let plan = host
        .store_coordination(
            ...
            PrismCoordinationArgs {
                ...
                payload: json!({
                    "title": "Git execution",
                    "goal": "Reject direct active task creation",
                    "policy": {
                        "gitExecution": {
                            "startMode": "require",
                            "completionMode": "require",
...
#[test]
fn git_execution_policy_completion_require_rejects_dirty_user_changes() {
    let branch = "task/git-execution-require";
    let root = init_git_workspace(branch);
    let session = index_workspace_session_with_options(
        &root,
        WorkspaceSessionOptions {
    ...
    assert!(error
        .to_string()
        .contains("completion mode `require` needs user changes committed before completion"));
...
#[test]
fn git_execution_policy_completion_require_publishes_after_manual_code_commit() {
    let branch = "task/git-execution-require-publish";
    let root = init_git_workspace(branch);
    let session = index_workspace_session_with_options(
        &root,
        WorkspaceSessionOptions {
    ...
    let plan = host
        .current_prism()
        .plan(&prism_ir::PlanId::new(plan.state["id"].as_str().unwrap()))
        .unwrap();
    assert_eq!(task.task.lifecycle_status, prism_ir::TaskLifecycleStatus::Completed);
    assert_eq!(task.status, prism_ir::EffectiveTaskStatus::Completed);
    assert_eq!(plan.status, prism_ir::DerivedPlanStatus::Completed);
    assert_eq!(
        task.task.git_execution.status,
    ...
    assert_eq!(
        task.task.git_execution.source_commit.as_deref(),
        Some(manual_commit.as_str())
    );
    assert_eq!(task.task.git_execution.source_ref.as_deref(), Some(branch));
    assert_eq!(
        task.task.git_execution.target_ref.as_deref(),
        Some("origin/main")
    );
    assert_eq!(task.task.git_execution.publish_ref.as_deref(), Some(branch));
    assert_eq!(
        task.task.git_execution.target_commit_at_publish.as_deref(),
    ...
    assert_eq!(task.task.git_execution.review_artifact_ref, None);
    assert_eq!(task.task.git_execution.integration_commit, None);
    assert_eq!(
        task.task.git_execution
            .last_publish
            .as_ref()
    ...
    assert_eq!(
        task.task.git_execution.publish_commit.as_deref(),
        task.task.git_execution
            .last_publish
            .as_ref()
            .and_then(|publish| publish.coordination_commit.as_deref())
...
fn git_execution_policy_completion_rehydrates_stale_plan_policy_before_publish() {
    ...
    let stale_plan = stale_host
        .current_prism()
        .plan(&prism_ir::PlanId::new(plan_id.clone()))
        .unwrap();
    assert!(matches!(
        stale_plan.plan.policy.git_execution.completion_mode,
        prism_coordination::GitExecutionCompletionMode::Off
    ));
...
fn auto_pr_approved_review_advances_integration_and_allows_verified_landing() {
    ...
    assert_eq!(
        task.task.git_execution.integration_commit.as_deref(),
        Some(target_commit.as_str())
    );
    assert_eq!(
        task.task.git_execution.review_artifact_ref.as_deref(),
...
fn git_execution_policy_completion_require_push_failure_keeps_task_in_progress() {
    ...
    assert_eq!(task.status, prism_ir::EffectiveTaskStatus::Active);
    assert!(
        !task.graph_actionable,
        "task after failed publish should remain non-actionable: {task:?}"
    );
    assert_eq!(
        task.task.git_execution.status,
        prism_ir::GitExecutionStatus::PublishFailed
...
fn plan_node_mutations_return_graph_native_views() {
    ...
    let prism = host.current_prism();
    prism.replace_coordination_snapshot_with_runtime_descriptors(
        prism_coordination::coordination_snapshot_from_plan_graphs(&[prism_ir::PlanGraph {
...
fn plan_edge_mutations_update_projected_dependency_graph() {
    ...
    let graph = prism_coordination::snapshot_plan_graphs(&host.current_prism().coordination_snapshot())
        .into_iter()
        .find(|graph| graph.id == PlanId::new(plan_id.clone()))
        .expect("plan graph");
    assert!(!graph.edges.iter().any(|edge| edge.from.0 == node_id
        && edge.to.0 == dependency_id
        && edge.kind == PlanEdgeKind::DependsOn));
...
fn plan_query_reads_surface_native_ready_nodes_and_blockers() {
    ...
    let ready_tasks = execution
        .dispatch("readyTasks", &format!(r#"{{ "planId": "{plan_id}" }}"#))
        .unwrap();
...
fn mcp_plan_create_and_update_surface_scheduling_metadata() {
                ...
                payload: json!({
                    ...
                    "policy": {
                        "gitExecution": {
                            ...
                            "targetBranch": "main",
                            "requireTaskBranch": true,
                            "maxCommitsBehindTarget": 2,
                            "maxFetchAgeSeconds": 300
                        }
                    },
                    "scheduling": {
                        "importance": 40,
    ...
    assert_eq!(created.state["gitExecutionPolicy"]["startMode"], "require");
    assert_eq!(
        created.state["gitExecutionPolicy"]["completionMode"],
        "require"
    );
    assert_eq!(
        created.state["gitExecutionPolicy"]["targetRef"],
        "origin/main"
    );
    assert_eq!(
        created.state["gitExecutionPolicy"]["maxCommitsBehindTarget"],
        2
    );
    assert_eq!(
        created.state["gitExecutionPolicy"]["maxFetchAgeSeconds"],
        300
    );
    ...
    assert_eq!(
        updated.state["gitExecutionPolicy"]["targetRef"],
        "refs/heads/release"
    );
    assert_eq!(
        updated.state["gitExecutionPolicy"]["maxCommitsBehindTarget"],
        0
    );
    assert_eq!(
        updated.state["gitExecutionPolicy"]["maxFetchAgeSeconds"],
        120
    );
    ...
    assert_eq!(plans[0]["gitExecutionPolicy"]["startMode"], "require");
    assert_eq!(plans[0]["gitExecutionPolicy"]["completionMode"], "require");
    assert_eq!(
        plans[0]["gitExecutionPolicy"]["targetRef"],
        "refs/heads/release"
    );
    assert_eq!(plans[0]["gitExecutionPolicy"]["targetBranch"], "main");
    assert_eq!(plans[0]["gitExecutionPolicy"]["requireTaskBranch"], true);
    assert_eq!(plans[0]["gitExecutionPolicy"]["maxCommitsBehindTarget"], 0);
    assert_eq!(plans[0]["gitExecutionPolicy"]["maxFetchAgeSeconds"], 120);
}
...
fn mcp_plan_update_rehydrates_stale_coordination_runtime_before_mutating() {
    ...
    let prism = host.current_prism();
    assert!(
        prism.plan(&plan_id).is_some(),
        "plan runtime should still expose the published plan"
    );
...
#[test]
fn mcp_plan_update_accepts_archived_status_and_archived_plan_rejects_new_claims() {
    let root = temp_workspace();
    let host = QueryHost::with_session(index_workspace_session(&root).unwrap());
...
fn mcp_plan_archive_terminalizes_active_task_execution_plan_before_archiving() {
    ...
    assert_eq!(
        host.current_prism()
            .plan(&prism_ir::PlanId::new(plan_id))
            .unwrap()
            .status,
        prism_ir::DerivedPlanStatus::Archived
    );
}
...
fn validation_plan_accepts_native_plan_node_task_ids() {
    ...
    let prism = host.current_prism();
    prism.replace_coordination_snapshot_with_runtime_descriptors(
        prism_coordination::coordination_snapshot_from_plan_graphs(
            &[native_graph],
            &std::collections::BTreeMap::new(),
        ),
        Vec::new(),
...
fn coordination_inbox_and_task_brief_share_authoritative_task_backed_status() {
    ...
    assert_eq!(
        envelope.result["inbox"]["plan"]["id"],
        Value::String(plan_id.0.to_string())
    );
    assert_eq!(
        envelope.result["inbox"]["children"]["children"][0]["id"],
...
fn coordination_schema_surfaces_closed_status_and_kind_enums() {
    ...
    assert!(task_create_schema.contains("\"kind\""));
    assert!(task_create_schema.contains("\"investigate\""));
    assert!(task_create_schema.contains("\"edit\""));
...
#[test]
fn spec_drift_validation_matrix_covers_current_spec_doc_patterns() {
    let root = temp_workspace();
    write_spec_drift_validation_matrix_workspace(&root);
    let host = QueryHost::with_session(index_workspace_session(&root).unwrap());
    let cases = [
        ("docs/SPEC.md", "11.2 Compact Default Tools", "prism_locate"),
        (
            "docs/DASHBOARD_IMPLEMENTATION_SPEC.md",
            "Validation view",
            "validation_feedback_view",
        ),
        (
        ...
        (
            "docs/PRISM_FIRST_CLASS_PLANS_SPEC.md",
        ...
        (
            ...
            "query_surface_validation_report",
        ),
    ];

    let contains_fragment = |value: &Value, fragment: &str| {
        value["path"]
            .as_str()
    ...
    for (path, heading, expected_fragment) in cases {
        let query = format!(
            r#"
const spec = prism.search({heading:?}, {{
  path: {path:?},
  kind: "markdown-heading",
  limit: 1,
}})[0];
return spec ? {{
  cluster: prism.specCluster(spec),
  drift: prism.explainDrift(spec),
}} : null;
"#
        );
        let result = host
            .execute(test_session(&host), &query, QueryLanguage::Ts)
            .expect("query should succeed");
        assert!(
            !result.result.is_null(),
            "expected a spec heading match for {path} / {heading}"
        );
        assert!(result.result["cluster"]["spec"].is_object());
        assert!(result.result["drift"]["cluster"].is_object());
        assert!(result.result["drift"]["expectations"]
            .as_array()
            .is_some_and(|items| !items.is_empty()));
        assert!(result.result["drift"]["observations"]
            .as_array()
            .is_some_and(|items| !items.is_empty()));
        assert!(result.result["drift"]["nextReads"]
            .as_array()
            .is_some_and(|items| !items.is_empty()));
        assert!(result.result["cluster"]["tests"]
            .as_array()
            .is_some_and(|items| !items.is_empty()));

        let surfaced = [
            &result.result["cluster"]["implementations"],
            &result.result["cluster"]["validations"],
            &result.result["cluster"]["related"],
            &result.result["cluster"]["readPath"],
            &result.result["cluster"]["writePath"],
            &result.result["cluster"]["persistencePath"],
            &result.result["cluster"]["tests"],
            &result.result["drift"]["nextReads"],
        ]
        .into_iter()
        .any(|value| {
            value.as_array().is_some_and(|items| {
                items
                    .iter()
                    .any(|item| contains_fragment(item, expected_fragment))
            })
        });
        assert!(
            surfaced,
            "expected {path} / {heading} to surface {expected_fragment}"
        );
    }
}
            ...
            meta: EventMeta {
                ...
                execution_context: Some(EventExecutionContext {
                    ...
                    work_context: Some(WorkContextSnapshot {
                        ...
                        summary: Some("Exercise mutation provenance stamping.".to_string()),
                        parent_work_id: None,
            ...
            meta: EventMeta {
                ...
                execution_context: Some(EventExecutionContext {
                    ...
                    work_context: Some(WorkContextSnapshot {
                        ...
                        summary: Some("Exercise bounded mutation provenance stamping.".to_string()),
                        parent_work_id: None,
...
fn hold_runtime_sync_lock_for(
    ...
    let handle = thread::spawn(move || {
        ...
        ready_tx
            .send(())
            .expect("lock holder should signal readiness");
        thread::sleep(duration);
    });
    ready_rx
        .recv_timeout(Duration::from_secs(1))
        .expect("lock holder should acquire the sync lock");
    handle
}

#[test]
fn persisted_only_mutations_wait_for_runtime_sync_and_then_succeed() {
    let root = temp_workspace();
    let server = PrismMcpServer::with_session_and_features(
        index_workspace_session(&root).unwrap(),
        PrismMcpFeatures::full().with_internal_developer(true),
    ...
    let result = server
        ...
        .expect("persisted-only mutation should wait for runtime sync and succeed");
    release.join().expect("lock holder thread should finish");

    assert!(
        started.elapsed() >= Duration::from_millis(50),
        "persisted-only mutation should have waited briefly on the runtime sync lock"
    );
    assert!(
    ...
    assert!(
        args.get("metrics")
            .and_then(Value::as_object)
            .and_then(|metrics| metrics.get("lockWaitMs"))
...
#[test]
fn coordination_mutations_wait_for_runtime_sync_and_then_succeed() {
    let root = temp_workspace();
    let server = PrismMcpServer::with_session_and_features(
        index_workspace_session(&root).unwrap(),
        PrismMcpFeatures::full().with_internal_developer(true),
    ...
    assert!(
        started.elapsed() >= Duration::from_millis(50),
        "coordination mutation should have waited briefly on the runtime sync lock"
    );
    assert!(
        started.elapsed() < Duration::from_millis(1500),
...
fn plans_resource_payload_surfaces_filters_and_root_nodes() {
    ...
    let plan = host
        .store_coordination(
            ...
            PrismCoordinationArgs {
                kind: CoordinationMutationKindInput::PlanCreate,
                payload: json!({
                    "title": "Migrate persistence storage semantics",
                    "goal": "Migrate persistence storage semantics",
                    "policy": {
                        "gitExecution": {
                            "startMode": "require",
                            "completionMode": "require",
                            ...
                            "targetBranch": "main",
                            "requireTaskBranch": true,
                            "maxCommitsBehindTarget": 1,
                            "maxFetchAgeSeconds": 600
                        }
                    }
                }),
                task_id: None,
    ...
    assert_eq!(
        ...
        vec![root_task.state["id"].as_str().unwrap()]
    );
    ...
    assert_eq!(
        payload.plans[0].git_execution_policy.target_ref.as_deref(),
        Some("origin/main")
    );
    assert_eq!(payload.plans[0].git_execution_policy.target_branch, "main");
    assert!(payload.plans[0].git_execution_policy.require_task_branch);
    assert_eq!(
        payload.plans[0]
    ...
    assert_eq!(
        payload.plans[0].git_execution_policy.max_fetch_age_seconds,
        Some(600)
    );
}
...
fn plan_resource_payload_surfaces_detail_summary_and_navigation_links() {
    ...
    assert_eq!(
        payload.plan.git_execution_policy.target_ref.as_deref(),
        Some("origin/main")
    );
    assert_eq!(payload.plan.git_execution_policy.target_branch, "main");
    assert!(payload.plan.git_execution_policy.require_task_branch);
    assert_eq!(
        payload
            .summary
            .as_ref()
...
#[test]
fn start_task_can_bind_directly_to_task_record() {
    let root = temp_workspace();
    let host = QueryHost::with_session(index_workspace_session(&root).unwrap());
    ...
    assert_eq!(replay.events[0].metadata["taskId"], task_id);
    ...
    assert_eq!(
        session
            .current_task
            .as_ref()
            .map(|task| task.context_status.as_str()),
        Some("active")
    );
}

#[test]
fn session_resource_surfaces_detached_current_task_context() {
    let host = host_with_node(demo_node());
...
#[test]
fn session_resource_surfaces_stale_current_task_context() {
    let mut graph = Graph::new();
    let alpha = NodeId::new("demo", "demo::alpha", NodeKind::Function);
    graph.add_node(Node {
        id: alpha.clone(),
    ...
    history.apply(&ObservedChangeSet {
        meta: EventMeta {
            id: EventId::new("observed:session-resource-stale"),
            ts: 1,
            actor: EventActor::System,
            correlation: None,
            causation: None,
    ...
    let (plan_id, _) = coordination
        .create_plan(
            EventMeta {
                id: EventId::new("coord:plan:session-resource-stale"),
                ts: 1,
                actor: EventActor::Agent,
                correlation: None,
                causation: None,
    ...
    let (task_id, _) = coordination
        .create_task(
            EventMeta {
                id: EventId::new("coord:task:session-resource-stale"),
                ts: 2,
                actor: EventActor::Agent,
                correlation: None,
                causation: None,
