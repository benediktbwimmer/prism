...
use prism_coordination::{
    CanonicalTaskRecord, CoordinationTask, GitExecutionCompletionMode, GitExecutionStartMode,
...
fn coordination_task_ref(
    ...
    match (&payload.task_id, &payload.client_task_id) {
        (Some(task_id), None) => Ok(CoordinationTransactionTaskRef::Id(
...
impl QueryHost {
    ...
    fn maybe_handle_git_execution_coordination(
        ...
        let plan = current_coordination_plan_view(prism.as_ref(), &task.plan)?;
                ...
                let input = CoordinationTransactionInput {
                    mutations: payload
                        ...
                        .map(|mutation| match mutation {
                            ...
                            CoordinationTransactionMutationPayload::TaskCreate(payload) => {
                            ...
                            CoordinationTransactionMutationPayload::DependencyCreate(payload) => {
        ...
        match (args.task_id, args.claim_id) {
            (Some(task_id), None) => {
                ...
                Ok(HeartbeatLeaseMutationResult {
                    ...
                    rejected: false,
                    violations: Vec::new(),
                    state: current_coordination_task_state(prism, &task.id)?,
                })
            }
            (None, Some(claim_id)) => {
                let claim = prism.renew_native_claim(
