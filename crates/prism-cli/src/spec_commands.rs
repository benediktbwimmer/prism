...
use prism_core::{
    ...
    SqliteSpecMaterializedStore, WorkspaceSession,
};
    ...
    F: FnOnce(&dyn SpecQueryEngine) -> Result<T>,
{
    ...
    refresh_spec_materialization(&store, root, Some(session.prism().coordination_snapshot()))?;
    let engine = MaterializedSpecQueryEngine::new(&store);
    f(&engine)
}

fn spec_materialized_db_path(root: &Path) -> PathBuf {
    root.join(".prism")
        .join("state")
