...
impl Graph {
    ...
    pub fn portable_path(&self, path: &Path) -> PathBuf {
        self.normalize_input_path(path)
    }

    pub fn runtime_path(&self, path: &Path) -> PathBuf {
        self.runtime_path_for_stored(&self.normalize_input_path(path))
    }
    ...
    fn upsert_file_from_with_observed_internal(
        ...
        let normalized_previous_path =
            previous_path.map(|candidate| self.normalize_input_path(candidate));
