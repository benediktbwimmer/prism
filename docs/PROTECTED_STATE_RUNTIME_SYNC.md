...
- The normal source watcher ignores `.prism` entirely in [`watch.rs`](/Users/bene/code/prism/crates/prism-core/src/watch.rs#L978).
- Memory is a special case: some session read paths opportunistically import repo memory events through [`sync_repo_memory_events_locked(...)`](/Users/bene/code/prism/crates/prism-core/src/session.rs#L3068).
...
The existing source watcher excludes `.prism` by policy in [`watch.rs`](/Users/bene/code/prism/crates/prism-core/src/watch.rs#L978).
...
Several session reads opportunistically call [`sync_repo_memory_events_locked(...)`](/Users/bene/code/prism/crates/prism-core/src/session.rs#L3068), for example in [`load_episodic_snapshot(...)`](/Users/bene/code/prism/crates/prism-core/src/session.rs#L1242).
...
That preserves the rule from [`REFRESH_RUNTIME_REDESIGN.md`](/Users/bene/code/prism/docs/REFRESH_RUNTIME_REDESIGN.md#L182): writes should not rely on "persist then reload to see your write."
...
TODO-PSYNC-2: Remove all session read-path calls to `sync_repo_memory_events_locked(...)` in [`session.rs`](/Users/bene/code/prism/crates/prism-core/src/session.rs).
...
TODO-PSYNC-4: Introduce a dedicated protected-state watcher for `.prism/**` that is separate from the normal source watcher in [`watch.rs`](/Users/bene/code/prism/crates/prism-core/src/watch.rs).
