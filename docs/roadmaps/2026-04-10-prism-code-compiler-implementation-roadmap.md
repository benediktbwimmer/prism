...
- The coordination boundary now accepts `structuredTransaction` payloads from the compiler-owned
  write runtime, executes the structured region/member tree directly instead of flattening it into
  a precomputed mutation list, and stamps the durable structure onto committed events as
  `transactionStructure`.
- The generated `prism_code` write surface no longer depends on a catalog of named host write
  operations like `__coordinationCreatePlan` and `__claimAcquire`; the live product path now routes
  writes through a single compiler-owned `__compilerWrite` host boundary keyed by method path.
- The remaining flat mutation fallback inside `prism-query` is now removed from the authority
  transaction contract: `CoordinationTransactionInput` requires structured execution, the
  authority-side transaction runner no longer accepts or iterates a flat `mutations` list, and
  internal `prism-query` convenience helpers now synthesize valid structured sequence regions
  instead of bypassing the structured contract.
- The authority boundary now derives durable `transactionStructure` metadata from structured
  execution even when no raw annotation payload was supplied, so the structured transaction is no
  longer an optional sidecar for successful writes.
- `prism.work.declare(...)` remains a host-managed session effect, but it now participates only via
  the same compiler-emitted structured transaction tree; there is no separate `sessionEffects`
  payload or non-structured product-path escape hatch left on the live compiler write path.
- The mixed structured executor now walks the compiler-emitted tree once, applies
  `prism.work.declare(...)` effects in tree order, and only then builds the authority transaction
  for subsequent coordination effects. This closes the prior ordering leak where declared-work
  effects were applied in a separate prepass before coordination lowering.
