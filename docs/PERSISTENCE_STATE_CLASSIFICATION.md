...
| Federated peer reads and shared-ref continuity across instances | `execute_remote_prism_query_resolves_runtime_id_from_shared_ref`, `shared_coordination_ref_live_sync_suppresses_self_write_and_imports_remote_change` | run two worktrees, publish a shared coordination update from one, and confirm the other resolves the peer by `runtime_id` and observes the shared-ref change without any shared SQLite dependency |
